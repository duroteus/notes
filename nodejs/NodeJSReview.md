...
Esse comportamento faz parte do modelo _at-least-once delivery_: a fila garante que a mensagem não será perdida, mesmo que precise entregar mais de uma vez.
...
Isso ocorre porque a maioria das filas usa _at-least-once delivery_, proiorizando não perder mensagens.
...
#### 36) O que é um health check e qual a diferença entre _liveness probe_ e _readiness probe_ em ambientes como Kubernetes?
...
**_Eventually consistent_** significa que, após uma operação, o sistema pode apresentar estados temporariamente diferentes entre serviços, mas todos convergem para o mesmo estado final após propagação dos eventos.
...
Outras estratégias incluem _stale-while-revalidate_ e adicionar _jitter_ ao TTL para evitar expiração simultânea.
